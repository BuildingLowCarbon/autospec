...
import { apiJson, normalize } from './dashboardApi';
...
export default function MappingsPage() {
  ...
  useEffect(() => { apiJson('/api/lignum/mappings').then((data) => { setRows(data.rows); setTargets(data.tbzItems); }).catch((e) => setMessage(`Erreur : ${e.message}`)); }, []);
  const targetById = useMemo(() => new Map(targets.map((item) => [String(item.id), item])), [targets]);
  const visible = useMemo(() => { const needle = normalize(query); return rows.filter((row) => (!onlyMissing || !row.tbzId) && (!needle || [row.lignumId, localizedName(row, lang), row.workingtitle, row.kbobId, row.tbzId, localizedName(targetById.get(String(row.tbzId)), lang)].some((v) => normalize(v).includes(needle)))); }, [lang, onlyMissing, query, rows, targetById]);
  ...
  return <section className="db-card">
    <div className="db-title-row"><div><h2>{t.mappings} Lignum → TBZ</h2><p>{t.mappingIntro}</p></div><button className="db-primary" disabled={busy} onClick={save}>{t.saveMappings}</button></div>
