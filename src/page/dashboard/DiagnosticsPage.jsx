...
export default function DiagnosticsPage() {
  ...
  const reasonLabel = (reason) => ({
    missing_mapping: t.missingMapping,
    invalid_mapping_target: t.invalidMappingTarget,
    unknown_reference: t.unknownReference,
  }[reason] || reason);
  useEffect(() => {
    load();
  ...
  return <div className="db-stack">
    ...
    <section className="db-card"><h3>{t.unresolvedReferences}</h3>{data && !data.unresolved.length ? <p className="db-success">{t.allReferencesResolved}</p> : data?.unresolved?.map((group) => <details className="db-problem" key={group.referenceId}><summary><span><code>{group.referenceId}</code>{group.mappedId ? <> → <code>{group.mappedId}</code></> : null}</span><span>{reasonLabel(group.reason)} — {group.occurrences.length} {t.occurrences}</span></summary><div>{group.occurrences.map((item, i) => <p key={`${item.componentId}-${i}`}><strong>{item.serialNo || item.componentId}</strong> · {labelOf(item, lang)} · {t.layer} {item.layer + 1} « {labelOf({ translations: item.layerTranslations, workingtitle: item.layerName }, lang) || '—'} »</p>)}</div></details>)}</section>
  </div>;
}
